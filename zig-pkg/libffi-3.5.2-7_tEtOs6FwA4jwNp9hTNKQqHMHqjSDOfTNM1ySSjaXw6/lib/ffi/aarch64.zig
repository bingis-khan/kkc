...
const arg_longlong = builtin.target.os.tag == .windows;
...
pub const Function = if (builtin.target.os.tag.isDarwin() or builtin.target.os.tag == .windows) extern struct {
    abi: Abi,
    param_count: c_uint,
...
pub const Closure = if (builtin.target.os.tag.isDarwin() and builtin.target.cpu.arch == .aarch64) extern struct {
