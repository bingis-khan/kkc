...
const target = switch (builtin.target.cpu.arch) {
    .aarch64, .aarch64_be => @import("ffi/aarch64.zig"),
    .arc => @import("ffi/arc.zig"),
    .arm, .armeb => @import("ffi/arm.zig"),
    .csky => @import("ffi/csky.zig"),
    .loongarch64 => @import("ffi/loongarch64.zig"),
    .m68k => @import("ffi/m68k.zig"),
    .mips, .mipsel, .mips64, .mips64el => @import("ffi/mips.zig"),
    .powerpc, .powerpcle, .powerpc64, .powerpc64le => @import("ffi/powerpc.zig"),
    .riscv32, .riscv64 => @import("ffi/riscv.zig"),
    .s390x => @import("ffi/s390x.zig"),
    .sparc, .sparc64 => @import("ffi/sparc.zig"),
    .x86, .x86_64 => @import("ffi/x86.zig"),
    .xtensa => @import("ffi/xtensa.zig"),
    else => unreachable,
};
...
pub const have_long_double = if (@hasDecl(target, "have_long_double"))
    ...
    builtin.target.cTypeByteSize(.longdouble).? > builtin.target.cTypeByteSize(.double).?;
...
pub const types = struct {
    ...
    pub const uint = switch (builtin.target.cTypeBitSize(.uint)) {
        16 => &ffi_type_uint16,
        32 => &ffi_type_uint32,
        else => unreachable,
    };
    pub const sint = switch (builtin.target.cTypeBitSize(.int).?) {
        16 => &ffi_type_sint16,
        32 => &ffi_type_sint32,
        else => unreachable,
    };
    pub const ulong = switch (builtin.target.cTypeBitSize(.ulong).?) {
        32 => &ffi_type_uint32,
        64 => &ffi_type_uint64,
        else => unreachable,
    };
    pub const long = switch (builtin.target.cTypeBitSize(.long)) {
        32 => &ffi_type_sint32,
        64 => &ffi_type_sint64,
        else => unreachable,
    };
...
pub extern fn ffi_get_version_number() c_ulong;

pub extern fn ffi_get_version() [*:0]const c_char;

pub extern fn ffi_get_default_abi() c_uint;
...
pub extern fn ffi_get_closure_size() usize;
