...
topLevels: std.ArrayList(AST.Function.Use),
...
const Export = struct {
    const ConsDef = union(enum) {
        Cons: []Token,
        AllCons,

        fn explicitlyHasConstructors(self: @This()) bool {
            return switch (self) {
                .Cons => |cons| cons.len > 0,
                .AllCons => false,
            };
        }
    };
...
pub fn parse(self: *Self) !Module {
    ...
    if (self.check(.EXPORT)) {
        ...
        if (self.check(.INDENT)) {
            ...
            while (true) {
                // maybe disallow indentation in a single export?
                const endedInIndent = try self.oneHotExport(&definedExports, &.{});
...
fn typeSynonym(self: *Self, typename: Token, tvarToks: []Token, annotations: []AST.Annotation) !void {
    ...
    const name = typename.literal(self.lexer.source);
    var tvars = try std.ArrayList(AST.TVarOrNum).initCapacity(self.arena, tvarToks.len);
    ...
    try self.newTypeSynonym(name, synonym);
}
...
fn namedRecordDefinition(self: *Self, modpath: Module.Path, name: Token) !*AST.Expr {
    ...
    if (mDataOrClass) |dataOrClass| {
        switch (dataOrClass) {
            ...
            .Synonym => unreachable, // TODO (I allow it, but todo implement)
            .Class => {
...
fn constructorExpression(self: *Self, modpath: Module.Path, name: Token) !*AST.Expr {
    ...
    const t = if (ct.tys.len == 0) ct.t else try self.typeContext.newType(.{
        .Fun = .{
            .args = ct.tys,
            .ret = ct.t,
            .env = try self.typeContext.newEnv(try TypeContext.Env.empty(self.gen.envs.newUnique(), self.arena)), // nocheckin: we have to figure out if the env is the same.
        },
    });
    return self.allocExpr(.{
        .e = .{ .Con = ct.con },
...
fn newVar(self: *@This(), varTok: Token, deconUse: ?AST.DeconUse) !Module.VarAndType {
    ...
    try self.scope.currentScope().vars.put(varName, .{ .thing = .{ .Var = .{
...
fn newFunction(self: *@This(), funNameTok: Token) !*AST.Function {
    ...
    try self.scope.currentScope().vars.put(varName, .{ .thing = .{ .Fun = funPtr } });
    return funPtr;
}
    ...
    if (modpath.len == 0) {
        ...
        while (lastVars.nextPtr()) |cursc| {
            if (cursc.vars.get(varName)) |vorf| {
                return .{ .vorf = vorf.thing, .level = lvl };
            }
            lvl -= 1;
        }
    } else {
        // TODO: better error when the variable is not defined in the module (currently has error "undefined variable")
...
fn instantiateClassFunction(self: *Self, cfun: *AST.ClassFun, l: Loc) !struct {
    ...
    // NOTE: everywhere this function was called we want to add the "USE"
    const use = AST.Function.Use{
        .ClassFun = .{
            .cfun = cfun,
            .ref = ref,
            .t = funTy,
                ...
                .Con => |con| {
                    if (if (assoc.instances.getPtr(assoc.class)) |dataInstances| dataInstances.get(con.type) else null) |inst| errexit: {
                        if (assoc.concrete) |conc| {
                            ...
                            // if top level, add inst
                            // switch (conc.envType) {
                            //     .ClassFunInstantiation => {
                            //         if (AST.EnvFun.getFun(conc.env)) |afun| {
                            //             try afun.temp__uses.append(envInst);
                            //         } else {
                            //             try self.topLevels.append(envInst);
                            //         }
                            //     },
                            ...
                            //     },
                            // }
                            // if (fun.env.outer == null and conc.envType == .ClassFunInstantiation and conc.env == null) {
                            //     try self.topLevels.append(envInst);
                            // }
                        } else {
                            // nothing. it's good.
...
fn newData(self: *@This(), data: *AST.Data) !void {
    // add type
    try self.scope.currentScope().types.put(data.name, .{ .thing = .{ .Data = data } });

    // add constructors
    switch (data.stuff) {
        .cons => |cons| {
            for (cons) |*con| {
                try self.scope.currentScope().cons.put(con.name, .{ .thing = con });
            }
        },
...
fn newClass(self: *Self, class: *AST.Class) !void {
    ...
    for (class.classFuns) |classFun| {
        try self.scope.currentScope().vars.put(classFun.name.name, .{ .thing = .{ .ClassFun = classFun } });
    }
}

fn newTypeSynonym(self: *Self, name: Str, synonym: *AST.TypeSynonym) !void {
    try self.scope.currentScope().types.put(name, .{ .thing = .{ .Synonym = synonym } });
}
...
fn newTNum(self: *Self, name: Str, binding: ?AST.Binding) !AST.TNum {
    ...
    try self.scope.currentScope().vars.put(name, .{ .thing = .{ .TNum = tnum } });
    return tnum;
}
...
pub const Association = struct {
    ...
    concrete: ?struct {
        ...
        env: ?AST.EnvFun, // fyi: this is self.env in class function instantiation and fun.env.outer when it's AssociatedInstantiation (when generalized)
        ...
        match: *const AST.Match, // this is funny - not sure if it should be here, as it is only used for the class match (because we are adding it later.)
...
const CurrentScope = struct {
    vars: std.StringHashMap(Import(Module.VarOrFun)),
    types: std.StringHashMap(Import(Module.DataOrClass)),
    cons: std.StringHashMap(Import(*AST.Con)),
    tvars: std.StringHashMap(AST.TVarOrNum),
    instances: std.AutoHashMap(*AST.Class, Module.DataInstance),

    fn Import(t: type) type {
        return struct {
            thing: t,
            fromWhere: enum { DefinedLocally, Imported } = .DefinedLocally,
        };
    }

    fn init(al: std.mem.Allocator) @This() {
        return .{
            .vars = std.StringHashMap(Import(Module.VarOrFun)).init(al),
            .types = std.StringHashMap(Import(Module.DataOrClass)).init(al),
            .cons = std.StringHashMap(Import(*AST.Con)).init(al),
...
fn expectOneOf(self: *Self, comptime tts: []const TokenType) !Token {
    for (tts) |tt| {
        if (self.consume(tt)) |tok| {
            return tok;
        }
    }
